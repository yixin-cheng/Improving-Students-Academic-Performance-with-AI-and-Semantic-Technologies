"""
This file is designed for the LSTM implementation in dropout prediction in ADM degree.
"""
...
target_path='data/ADM_clean.csv'
ga_path = 'data/ADM_final.csv'

"""
perform pre-processing. Uncomment below when you try pre-processing.
"""
# Preprocessor(intial_path,target_path,'ADM')
...
for i in range (200):
    ...
    plt.ylabel('loss')
    plt.show()
    # plt.savefig('fig/ADM_loss.jpg')
    plt.figure()
    plt.plot(all_accuracy)
    plt.xlabel('epoch*time step')
    plt.ylabel('accuracy')
    plt.show()
    #plt.savefig('fig/ADM_acc.jpg')
