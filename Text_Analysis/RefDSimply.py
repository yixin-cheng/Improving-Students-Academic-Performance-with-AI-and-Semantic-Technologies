...
class SemRefD:
    ...
    def Wfunctions(self,conceptC,ConceptAB):
        ...
        elif self.wfunc == 'taxo':
            return self.Wtaxonomic(conceptC,ConceptAB)
        else:
            raise ValueError('Funcion de ponderacion no definida')

    def Wtaxonomic(self, conceptC, ConceptAB):
        #print conceptC + "++"  +ConceptAB

        tfidf = self.Wtfidf(conceptC, ConceptAB)
        if tfidf > 0.0:
            txan = TaxonomicAnalysis(conceptC,ConceptAB)
            res = txan.features()
            if res['ave_tax_simil']:
                return res['ave_tax_simil'] * tfidf
            else:
                return 0.0
        else:
            return 0.0
